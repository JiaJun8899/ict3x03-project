...
class UpdateUserAPIView(APIView):
    def put(self, request):
        ...
        emergency = EmergencyContactService.EmergencyContactService.getContactById(
            request.data["id"]
        )
        if emergency:
            nokData = {
            ...
            nokUpdateSuccess = NokService.NokService.updateNok(
                nokData, emergency["nok"]
            )
        ...
        if valid != None:
            ...
            success = UserService.UserService.updateUserProfile(
...
class SignUpEventAPIView(APIView):
    def post(self, request):
        ...
        validEvent = EventCommonService.EventCommonService.getEventByID(
            request.data["eid"]
        )
        if validUser != None and validEvent != None:
            data = {"event": request.data["eid"], "participant": request.data["id"]}
